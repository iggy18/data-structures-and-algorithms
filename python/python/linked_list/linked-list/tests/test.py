...
def can_succesfully_instantiate_empty_linked_list():
    nothing = LinkedList()
    assert nothing.head == None


def can_properly_insert_into_linked_list():
...
def Head_properly_points_to_fist_node():
...
def can_properly_insert_multiple_nodes():
...
def return_true_when_finding_a_value_that_exists():
...
def return_false_when_searching_for_value_that_does_not_exsist():
    test_list = LinkedList()
...
def can_properly_return_collection_of_all_values_in_list():
    phrase = LinkedList()
    phrase.insert("a")
    phrase.insert("b")
    phrase.insert("c")
